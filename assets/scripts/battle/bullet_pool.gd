...
var bullet_textures: Dictionary[String, SpriteFrames] = {
	"default": SPRITE_FRAMES
}
var current_bullet_type: String = "default"

var pool_size: int = 1 << 14
...
var _active_index_map: Dictionary[int, int] = {}
...
func _ready() -> void:
	...
	_query.shape = SHAPE
	Performance.add_custom_monitor("game/bullet_count", get_active_bullets_count)

func get_active_bullets_count() -> int:
	return _active_bullets_count
...
func _process(delta: float) -> void:
	...
	for i in range(_active_bullets_count - 1, -1, -1):
		var bullet_index: int = _active_bullet_indices[i]
		var tx: Transform2D = transforms[bullet_index]
		var dir: Vector2 = Vector2.RIGHT.rotated(tx.get_rotation())
		...
		if _collision_result.size() > 0:
			var collider: PhysicsBody2D = _collision_result[0]["collider"]
...
func deactivate_bullet(index_to_deactivate: int) -> void:
	...
	var last_bullet_index: int = _active_bullet_indices[-1]
...
func clear_bullets() -> void:
	var indices_to_clear: Array = _active_index_map.keys()
	for bullet_index: int in indices_to_clear:
		deactivate_bullet(bullet_index)
