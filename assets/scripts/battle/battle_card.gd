...
func initialize(new_character: Player) -> void:
...
func setup_ability_icons() -> void:
	for icon: TextureRect in icons.values():
		icon.queue_free()
	icons.clear()
	...
	for i in range(character.abilities.size()):
		...
		cd_node.transition_pending.connect(
			func(total: float, current: float) -> void: gray_out_icon(total, current, node_base)
		)
...
func _exit_tree() -> void:
	for i in range(character.abilities.size()):
		...
		var cd_node := character.state_chart.get_node("Root/Battling/%s/Cooldown" % node_base)
		for connection: Dictionary in cd_node.transition_pending.get_connections():
