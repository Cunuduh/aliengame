...
func _process(delta: float) -> void:
	...
	if visible:
		...
		if result.size() > 0 and result[0]["collider"].is_in_group("enemy") and not _hit_targets.has(result[0]["collider"]):
			result[0]["collider"].stats.health -= Globals.player.stats.attack
			_hit_targets.append(result[0]["collider"])

func destroy() -> void:
	_queued_for_deletion = true
	var tween := get_tree().create_tween().set_parallel(true)
	tween.tween_property(self, "modulate", Color(1, 1, 1, 0), 0.25)
	tween.tween_property(self, "scale", Vector2(1, 0), 0.25)
